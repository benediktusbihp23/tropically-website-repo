...
export async function POST(
  ...
  console.log('[UPLOAD] Property ID:', params.id);
  ...
  try {
    ...
    const path = `${params.id}/${timestamp}-${random}.webp`;
    ...
    const orderUrl = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/property_images?property_id=eq.${params.id}&category_id=eq.${category_id}&order=display_order.desc&limit=1`;
    ...
    const dbResponse = await fetch(insertUrl, {
      ...
      body: JSON.stringify({
        property_id: parseInt(params.id),
